...
(CASE WHEN monthlymaintenance > 1000 THEN 'Expensive' ELSE 'Cheap' END) as expense_level
FROM `Facilities`
...
SELECT DISTINCT concat(surname, ', ', firstname) AS member_name, name AS court_name
FROM `Members`
INNER JOIN `Bookings`
USING (memid)
...
/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */
        ...
        SELECT m1.surname || ", " || m1.firstname AS member_name, m2.surname || ", " || m2.firstname AS rec_name
        FROM members AS m1
...
/* Q12: Find the facilities with their usage by member, but not guests */
        ...
        SELECT f.name, COUNT(b.facid) AS usage
...
/* Q13: Find the facilities usage by month, but not guests */
        ...
        SELECT f.name, COUNT(STRFTIME("%m", b.starttime)) AS usage
        FROM facilities AS f
        LEFT JOIN bookings AS b
        USING (facid)
        WHERE b.memid > 0
